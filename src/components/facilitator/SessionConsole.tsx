          ...
          <div className="space-y-1">
            <button
              ...
              className={`w-full px-4 py-2 text-white rounded-lg focus-visible-ring transition-colors duration-200 ${
                showEndConfirm ? 'bg-red-800 font-bold' : 'bg-red-600 hover:bg-red-700'
              }`}
              aria-live="polite"
              aria-describedby={showEndConfirm ? 'end-session-desc' : undefined}
            >
              {showEndConfirm ? '⚠️ Confirm End Session' : 'End Session'}
            </button>
            {showEndConfirm && (
              <p id="end-session-desc" className="text-sm text-red-600 text-center font-medium">
