        ...
        <span aria-hidden="true">✍️</span> Your Response
          ...
          <div>
            ...
            <div
              ...
              className={`text-right text-xs mt-1 ${alternativeThought.length >= 270 ? 'text-red-600 font-bold' : alternativeThought.length >= 240 ? 'text-orange-700 font-medium' : 'text-gray-500'}`}
            >
              {alternativeThought.length} / 300 characters
            </div>
            {alternativeThought.length >= 240 && alternativeThought.length < 270 && <div role="status" className="sr-only">Approaching character limit</div>}
            {alternativeThought.length >= 270 && alternativeThought.length < 300 && <div role="status" className="sr-only">Character limit almost reached</div>}
            {alternativeThought.length === 300 && <div role="status" className="sr-only">Character limit reached</div>}
          </div>
