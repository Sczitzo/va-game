...
## 2026-05-12 - Non-blocking confirm dialogs
**Learning:** Native `window.confirm` dialogs block the entire thread and can feel jarring. Replacing them with a state-based inline confirmation pattern (e.g., changing a button's style and text temporarily to prompt for a second click) improves UX significantly while avoiding blocking modals.
**Action:** Always prefer soft confirmation patterns that require double intent for destructive actions rather than relying on native `window.confirm` or creating complex custom modal dialogs.
