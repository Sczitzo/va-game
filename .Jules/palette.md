...
## 2026-10-25 - Accessible Character Counters
**Learning:** Updating character counts on every keystroke with `aria-live` creates excessive noise for screen reader users. Dynamic visual counters should not use `aria-live`.
**Action:** Render a separate visually hidden element (`.sr-only`) with `role='status'` containing a static warning message only when the character limit is approached. Do not include dynamic variables in the status string.
