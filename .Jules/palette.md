...
## 2026-02-15 - Progressive Character Count Feedback
**Learning:** Applying `aria-live="polite"` to a character counter continuously creates too much noise. Screen readers announce every update.
**Action:** Only enable `aria-live="polite"` when the user approaches the limit (e.g., >80%) to warn them without constant chatter.
