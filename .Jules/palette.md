...
## 2026-11-04 - Accessible Progressive Character Limits
**Learning:** Adding `aria-live` to dynamic character limit counters creates severe screen reader noise on every keystroke. Progressive visual feedback (color changes at 80%/90% capacity) needs an accessible equivalent.
**Action:** Render a separate visually hidden `.sr-only` element with `role='status'` containing a static warning string that only conditionally mounts when a threshold (e.g., 80%, 90%, 100%) is crossed. Hide the dynamic number counter with `aria-hidden="true"`.
