...
## 2024-06-06 - Accessible Character Counters
**Learning:** Adding `aria-live` to a changing character count causes noisy, disruptive screen reader updates on every keystroke.
**Action:** Provide progressive visual feedback with colors, but for screen readers, render separate, static warning messages with `role="status"` only at critical thresholds (e.g., 80% and 100%).
