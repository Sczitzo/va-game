...
## 2026-11-01 - Accessible Character Counters
**Learning:** Using `aria-live` on a dynamically updating character counter creates noisy, character-by-character spam for screen readers.
**Action:** Instead of `aria-live` on the counter itself, conditionally render strictly static, visually hidden elements with `role="status"` at specific thresholds (e.g., "Approaching limit", "Limit reached") to provide meaningful, non-intrusive warnings.
