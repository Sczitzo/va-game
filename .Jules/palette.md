...
## 2024-05-29 - Accessible Soft Confirmations
**Learning:** Native `window.confirm` dialogs lack accessibility and are visually jarring. Extracting the warning text to a dynamically rendered element with `role="status"` below a state-based soft confirmation button ensures screen readers reliably announce the consequences of the destructive action.
**Action:** Replace `window.confirm` with state-based soft confirmations and ensure any original warning text is preserved and visibly rendered with proper ARIA roles.
