...
## 2024-05-24 - Screen Reader Quiet Character Limits
**Learning:** Updating a visible character counter that has `aria-live` on every keystroke spams screen reader users with noisy updates.
**Action:** Remove `aria-live` from the changing counter text and instead conditionally render strictly static visually hidden elements with `role="status"` at specific thresholds (e.g., >80% and 100%) to provide critical warnings quietly.
