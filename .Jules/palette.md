...
## 2026-03-14 - Screen Reader Feedback for Changing Numbers
**Learning:** Adding `aria-live` directly to a character counter that changes with every keystroke causes noisy, overwhelming screen reader updates.
**Action:** Render a separate visually hidden (`clip:rect(0,0,0,0)`) element with `role="status"` that only appears conditionally when a threshold (like 80% capacity) is reached, providing a single, helpful warning instead of constant chatter.
