...
## 2026-10-25 - Accessible Dynamic Feedback
**Learning:** Using `aria-live` on a frequently updating character counter text causes a noisy and frustrating experience for screen reader users.
**Action:** Instead of applying `aria-live` to the changing number text, dynamically render completely separate, visually hidden (e.g., `.sr-only`) `<div role="status">` elements with static warning messages at specific thresholds (e.g., 80%, 90%, 100%) to provide meaningful, progressive feedback.
