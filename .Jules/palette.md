...
## 2026-10-25 - Accessible Dynamic Character Limits
**Learning:** Applying `aria-live` to dynamic character counts causes screen readers to constantly announce every keystroke when approaching a limit, causing excessive noise.
**Action:** When implementing progressive character counter feedback, use a separate visually hidden `.sr-only` element with `role="status"` that contains a static warning message only when a threshold is crossed (e.g. >80%). This provides accessible warnings without continuous interruptions.
