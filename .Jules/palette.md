...
**Learning:** When replacing native `window.confirm()` dialogs with state-based soft confirmations, the context/warning text from the original dialog is often lost.
**Action:** Ensure the warning text (detailing consequences) is explicitly rendered near the button with `role="status"` so it is visually present and reliably announced by screen readers when the state changes.
