...
**Learning:** Replacing native `window.confirm()` dialogs with state-based soft confirmations requires preserving contextual warnings dynamically below the button with `role="status"` so screen readers announce the consequences.
**Action:** Use a short timeout window and a visible ARIA live region to replace blocking modals for destructive actions.
