...
## 2025-04-29 - Preserving Context in Soft Confirms
**Learning:** When replacing a native `confirm()` with a soft-confirm inline state, the contextual warning text originally inside the native alert (e.g., "This will generate the session summary") must be preserved in the UI so users understand the consequences of the destructive action.
**Action:** Render a contextual warning paragraph (e.g., `<p>...</p>`) alongside the confirm button when the `showConfirm` state is true.
