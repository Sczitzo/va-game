...
## 2026-10-25 - Progressive Character Limit Warnings
**Learning:** Applying `aria-live` directly to a character counter causes noisy updates for screen reader users. Progressive visual coloring without accompanying static screen reader warnings leaves non-visual users unaware of impending limits.
**Action:** Conditionally render distinct, visually hidden elements with `role="status"` for "Approaching limit" and "Character limit reached" while using dynamic classes (`text-orange-700 font-medium`, `text-red-600 font-bold`) on the visual counter.
