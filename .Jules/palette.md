...
## 2026-03-01 - Progressive Character Counter Feedback
**Learning:** Using progressive visual colors and separate, statically rendered `role="status"` nodes provides safer and more precise screen reader announcements for character limits than noisy `aria-live` regions or ternary text swaps.
**Action:** Always implement character limit warnings with progressive visual cues (orange -> red) and conditionally render static screen reader elements that only trigger at precise mathematical boundaries (e.g., 100% capacity).
