...
**Learning:** Adding `aria-live` directly to a character counter causes noisy, continuous screen reader updates on every keystroke.
**Action:** Instead of making the counter live, conditionally render separate, strictly static hidden elements with `role="status"` at specific thresholds (e.g., 80% and 100%) to provide targeted, non-intrusive warnings without spamming the user.
