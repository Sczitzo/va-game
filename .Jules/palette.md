...
## 2024-10-25 - Extracted Warning from Destructive Modals
**Learning:** When replacing native `window.confirm()` dialogs with state-based soft confirmations, any secondary contextual information (like "This will generate the session summary") must be extracted and visibly rendered near the button with `role="status"` to maintain accessibility and ensure screen readers announce the consequence of the destructive action.
**Action:** Always extract and render contextual warning strings dynamically with `role="status"` when replacing native confirmation dialogs.
