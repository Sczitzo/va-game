...
**Learning:** For character limit counters, a static color and visually persistent text count update on every stroke creates noise for screen readers. Using `aria-live` on the counter causes extreme chattiness.
**Action:** Use conditional styling for visual feedback (e.g., orange at 80%, red and bold at 100%) and render entirely separate visually-hidden (`sr-only`) elements with static text ("Approaching character limit", "Character limit reached") using `role="status"` to ensure screen readers announce the warning precisely when the threshold is crossed, without spamming every keystroke.
