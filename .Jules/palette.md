...
## 2026-06-29 - Progressive Character Limit Feedback
**Learning:** Input fields with character limits need progressive visual feedback (color changes) and static screen reader announcements (using `role="status"` on visually hidden elements) instead of `aria-live` on the counter, to prevent noisy and confusing updates for assistive technologies.
**Action:** Implement color thresholds (80% warning, 100% danger) and conditionally render strictly static screen reader warnings (`sr-only` elements) when thresholds are crossed.
