...
**Learning:** Attaching `aria-live` to a dynamically updating character counter creates excessive noise for screen reader users on every keystroke.
**Action:** Use a separate visually hidden (`sr-only`) element with `role="status"` that conditionally renders *only* the threshold warnings (e.g., "Approaching limit", "Limit reached") without the dynamic numbers, while visually changing the visible counter's color for sighted users.
